...
func addPost(postImage: UIImage, thread: String, username: String) {
    ...
    // let fullPath: String = "\(firUsersNode)/\(id)/\(path)"
    dbRef.child("\(firUsersNode)/\(id)/\(firPostsNode)").childByAutoId().setValue(postDict)
...
func getPosts(user: CurrentUser, completion: @escaping ([Post]?) -> Void) {
    ...
    var postArray: [Post] = []
    let id: String = UUID().uuidString
    dbRef.child("\(firUsersNode)/\(id)/\(firPostsNode)").observeSingleEvent(of: .value, with: { snapshot -> Void in
        if snapshot.exists() {
                ...
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        ...
                        if ids.contains(postKey) {
                            newPost = Post(id: postKey, username: post!["username"] as! String, postImagePath: post!["imagePath"] as! String, thread: post!["thread"] as! String, dateString: post!["date"] as! String, read: true)
                        } else {
                            newPost = Post(id: postKey, username: post!["username"] as! String, postImagePath: post!["imagePath"] as! String, thread: post!["thread"] as! String, dateString: post!["date"] as! String, read: false)
                        }
